...
serve(async (req)=>{
  ...
  try {
    ...
    try {
      ...
      return new Response(JSON.stringify({
        error: `Template not found: ${type}.tex`
      }), {
        status: 400,
        headers: {
          'Content-Type': 'application/json',
    ...
    if (data.products) {
      const productRows = await Promise.all(data.products.map(async (p)=>{
        const { data: product, error } = await supabase.from('products').select('name, price, category_id, categories(name)').eq('id', p.product_id).eq('categories.id', 'category_id').single();
    ...
    const { error: uploadError } = await supabase.storage.from('documents').upload(`${type}s/${fileName.replace('.tex', '.pdf')}`, Deno.readFileSync(pdfPath), {
