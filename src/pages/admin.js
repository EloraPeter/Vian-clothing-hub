...
export default function AdminPage() {
  ...
  const [productData, setProductData] = useState({
    ...
    price: '',
    description: '',
    category_id: '', // Stores the selected category ID or new category name
    imageFiles: [],
  });
  ...
  const handleProductSubmit = async (e) => {
    ...
    try {
      ...
      // Check if the entered category_id is a new category name (not an ID)
      if (!categories.some((cat) => cat.id === parseInt(categoryId))) {
        ...
        if (newCategoryName) {
          ...
          categoryId = null; // Allow no category if input is empty
        }
      }
