...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error) {
      ...
      return res.status(500).json({ error: `Failed to send OTP email: ${error.message || 'Unknown Resend API error'}` });
    }
    ...
    return res.status(500).json({ error: `Unexpected error: ${err.message || 'Unknown server error'}` });
  }
}